...
// completion.
static inline void karatzuba(uint64_t *c,
                        const uint64_t *a,
                        const uint64_t *b,
                           ...
                           const gf2x_ctx *ctx)
{
  ...
  dbl_pad_r_t t = {0};
  uint64_t secure_buffer[SECURE_BUFFER_QWORDS] __align(LC_BIKE_ALIGN_BYTES);

  karatzuba((uint64_t *)&t, (const uint64_t *)a, (const uint64_t *)b, LC_BIKE_R_QWORDS,
            LC_BIKE_R_PADDED_QWORDS, secure_buffer, ctx);
