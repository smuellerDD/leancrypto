...
* Fix some locations with improper return code handling

* Rust: Fix lifetime issue
...
* ASN.1: fix potential memory leaks in ASN.1 in error code paths

* ASN.1: make pathlen verification completely consistent with RFC5280 sectoin 4.2.1.9

* Seeded RNG: add support for multiple instances with automatic selection of DRNG instance based on current CPU
...
* bug fix: GCM contains a buffer overflow potentially writing zeros beyond a caller buffer upon GCM decrypt with an oversized tag length (reported by Conner Webber)

* bug fix: Treat XDRBG initially seeded only after all seeding steps are successful (reported by Conner Webber)
