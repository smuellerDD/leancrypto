...
fn lc_rust_dilithium_ed25519() {
	unsafe {
		let mut sk: MaybeUninit<lc_dilithium_ed25519_sk> =
			MaybeUninit::uninit();
		let mut pk: MaybeUninit<lc_dilithium_ed25519_pk> =
			MaybeUninit::uninit();
		let mut sig: MaybeUninit<lc_dilithium_ed25519_sig> =
			MaybeUninit::uninit();

		let msg: [u8; 3] = [0x00, 0x01, 0x02];
		let msg2: [u8; 3] = [0x00, 0x01, 0x02];

		if lc_dilithium_ed25519_keypair(pk.as_mut_ptr(),
						sk.as_mut_ptr(),
						lc_seeded_rng) != 0 {
		...
		/* modify msg */
		let ret = lc_dilithium_ed25519_verify(sig.as_ptr(),
						      msg2.as_ptr(),
						      msg2.len(),
						      pk.as_ptr());
